...
@app.route('/series/watch/<series_name>/<season_name>/<episode_name>/')
def seriesWhatch(series_name, season_name, episode_name):
    with init_db() as con:
        cursor = con.cursor()
        cursor.execute('SELECT * FROM series WHERE name = ?', (series_name,))

        seriesName = cursor.fetchone()

    return render_template('series-watch.html', series_name=series_name, season_name=season_name, episode_name=episode_name, seriesName=seriesName)
